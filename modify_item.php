...
if (isset($_SESSION[$mod_name]['item']) && is_array($_SESSION[$mod_name]['item'])) {
	...
	$imgresize                    = $_SESSION[$mod_name]['item']['imgresize'];
	$resize_quality               = htmlspecialchars($_SESSION[$mod_name]['item']['quality']);
	$resize_maxheight             = htmlspecialchars($_SESSION[$mod_name]['item']['maxheight']);
	$resize_maxwidth              = htmlspecialchars($_SESSION[$mod_name]['item']['maxwidth']);
...
// Bugfix: WBCE provides jquery.ui v1.12.1 but plupload needs older v1.10.2
?>
...
<script src="<?php echo WB_URL; ?>/modules/<?php echo $mod_name; ?>/js/jquery/ui/jquery-ui.min.js"></script>
...
<table id="mod_<?php echo $mod_name; ?>_modify_item_b">
	<tr>
	...
	<tr>
		...
		<td>
			...
			<div class="datetimepickers_box_b">
				<span class="txt_scheduling"><?php echo $MOD_ONEFORALL[$mod_name]['TXT_SCHEDULING']; ?>:</span>
				<input type="text" class="datetimepicker" name="scheduling_start" id="scheduling_start" maxlength="30" value="<?php echo (isset($scheduling['start'])) ? $scheduling['start'] : ''; ?>">
				<input type="text" class="datetimepicker" name="scheduling_end" id="scheduling_end" maxlength="30" value="<?php echo (isset($scheduling['end'])) ? $scheduling['end'] : ''; ?>">
			</div>
			<?php } ?>
		</td>
	</tr>
...
// Show/hide item mover/duplicator (see general settings in the oneforall backend)
...
if ($query_fields->numRows() > 0) {
	while ($field = $query_fields->fetchRow()) {
		...
		$unserialized = __unserialize($value);
...
<table class="mod_<?php echo $mod_name; ?>_submit_table_b">
	<tr>
		<td>
			<input name="save_and_return" type="submit" value="<?php echo $TEXT['SAVE']; ?>">
			<input name="save" type="submit" value="<?php echo $MOD_ONEFORALL[$mod_name]['TXT_SAVE_AND_BACK_TO_LISTING']; ?>">
		</td>
		<td>
...
<div style="display: <?php echo $display_img_section; ?>">
	...
	<table id="mod_<?php echo $mod_name; ?>_images_b">
		<thead>
			<tr>
		...
		if ($query_image->numRows() > 0) {
			...
			<tr id="id_<?php echo $img_id; ?>">
			  ...
			  <td>
				<input type="text" name="images[<?php echo $img_id; ?>][title]" maxlength="255" value="<?php echo $image['title']; ?>">
				<input type="text" name="images[<?php echo $img_id; ?>][alt]" maxlength="255" value="<?php echo $image['alt']; ?>">
			  </td>
			  <td>
			    <textarea name="images[<?php echo $img_id; ?>][caption]" rows="3"><?php echo $image['caption']; ?></textarea>
			  </td>
			  <td>
	...
	<table class="mod_<?php echo $mod_name; ?>_submit_table_b">
		<tr>
			<td>
				<input name="save_and_return_to_images" type="submit" value="<?php echo $TEXT['SAVE']; ?>">
				<input name="save" type="submit" value="<?php echo $MOD_ONEFORALL[$mod_name]['TXT_SAVE_AND_BACK_TO_LISTING']; ?>">
			</td>
			<td>
				<input type="button" value="<?php echo $TEXT['CANCEL']; ?>" onclick="javascript: window.location = '<?php echo ADMIN_URL; ?>/pages/modify.php?page_id=<?php echo $page_id; ?>';">
	...
	// Multi runtime file uploader featuring drag&drop, resize images on clientside is not activated
	// http://www.plupload.com
	?>
	<a name="images"></a>
	<h2>3. <?php echo $TEXT['UPLOAD_FILES']; ?></h2>
	<div id="mod_<?php echo $mod_name; ?>_resize_settings_b">
		<input type="checkbox" name="imgresize" id="imgresize" value="true"<?php echo $imgresize == 'true' ? ' checked="checked"' : ''; ?>>
		<label for="imgresize"><?php echo $MOD_ONEFORALL[$mod_name]['TXT_IMAGE'].' '.$TEXT['RESIZE']; ?>:</label>
		<?php echo $MOD_ONEFORALL[$mod_name]['TXT_MAX_WIDTH']; ?><input type="text" size="5" name="maxwidth" value="<?php echo $resize_maxwidth; ?>">px
		<span></span>
		<?php echo $MOD_ONEFORALL[$mod_name]['TXT_MAX_HEIGHT']; ?><input type="text" size="5" name="maxheight" value="<?php echo $resize_maxheight; ?>">px
		<span></span>
		<?php echo $MOD_ONEFORALL[$mod_name]['TXT_JPG_QUALITY']; ?>: <input type="text" size="3" name="quality" value="<?php echo $resize_quality; ?>">
	</div>
	<div id="uploader">
